...
sources we have for the API. Work down this list:
 ...
 5. DONE: THE THREAT MODEL IS ITS OWN LLM CALL. llm_report() now calls
    llm_threat_model() at the end, a separate subagent turn driven by
    prompts.THREAT_MODEL (agent/prompts.py), verified against the live target
    with real Bedrock credentials -- not the model's SUMMARY line reused.
...
def llm_report(verdicts, target, session, points=None, elapsed=None):
    ...
    llm_threat_model(entry_points, confirmed, target, session)


# ---------------------------------------------------------------------------
# Threat model -- a separate LLM call over the same evidence (AGENTS.md
# section 7, Mike-4). Resolves note 5 in the module docstring: this used to be
# the stub's structure with the report subagent's summary pasted on top.
# ---------------------------------------------------------------------------
def _format_entry_points(points):
    return "\n".join("- %s %s %s" % (p["method"], p["param"], p["url"]) for p in points)


def _format_confirmed(hits):
    if not hits:
        return "(none confirmed in this run)"
    return "\n".join("- %s %s %s via %s" % (v["method"], v["param"], v["url"], v["technique"])
                     for v in hits)


def llm_threat_model(points, hits, target, session):
    """Stage 4b: the threat model, via its own subagent turn.

    Takes the same two facts llm_report already has -- the full entry-point
    list and the confirmed subset -- and asks a fresh model call to reason
    about trust boundaries and attacker value, instead of reusing the report
    subagent's SUMMARY line. Writes artifacts/02_threat_model.md; returns None.
    """
    user_text = (
        "Entry points enumerated on %s (%d total):\n\n%s\n\n"
        "Confirmed findings (%d):\n\n%s\n\n"
        "Write the threat model using the labelled markdown format from your "
        "instructions. List every entry point above under Entry points, and "
        "every confirmed finding above, unchanged, under Confirmed crossings."
        % (target, len(points), _format_entry_points(points),
           len(hits), _format_confirmed(hits)))

    tvars = _target_vars(target)
    text = _run_stage("03_threat_model", prompts.build(prompts.THREAT_MODEL, tvars),
                      user_text, build_tools(session), config.SUBAGENT_MODEL)

    # Models preface markdown with a line of chatter ("Here is the threat
    # model...") despite instructions not to. Strip anything before the
    # heading rather than ship a report that opens on a stray sentence.
    heading = text.find("# Threat model")
    if heading > 0:
        text = text[heading:]

    # A missing crossing is a bug in the model's prose, not a missing
    # vulnerability -- append what it dropped instead of trusting the
    # narrative was complete. Matched by URL, since that is what's in the text.
    ...
    if "# Threat model" not in text:
        raise ParseFailure(
            "threat model response carried no '# Threat model' heading. "
            "Model said: %s" % _snippet(text))
